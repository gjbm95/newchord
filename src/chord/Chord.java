...
public class Chord {
	...
	public static void main (String[] args) {
		...
		try {
                    ...
                    Nodo.obtenerInstancia().setDireccion(local_ip);
                    Nodo.obtenerInstancia().setPuertopeticion(Integer.parseInt(args[0]));
                    // determine if it's creating or joining a existing ring
                    // create, contact is this node itself
                    if (args.length == 2) {
                        SistemaUtil.servidorTiempo = args[1];
                        m_contact = m_node.getAddress();
                    }
                    
                    // join, contact is another node
                    else if (args.length == 4) {
                        SistemaUtil.servidorTiempo = args[3];
